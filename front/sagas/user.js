...
function regionAPI(regionData) {
  // 서버에 요청을 보내는 부분
  return axios.get('/', regionData);
}

function* region(action) {
  try {
    const result = yield call(regionAPI, action.data);
    yield put({ // put은 dispatch 동일
      type: REGION_SUCCESS,
      data: result.data,
    });
...
function* watchRegion() {
  yield throttle(2000, REGION_REQUEST, region);
}
...
function* unfollow(action) {
  try {
    ...
    const result = yield call(unfollowAPI, action.data, action.offset);
    yield put({ // put은 dispatch 동일
